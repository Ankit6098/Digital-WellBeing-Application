        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
                ...
                if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE && isCurrentlyActive) {
                    ...
                    Drawable deleteIcon = ContextCompat.getDrawable(getApplicationContext(), R.drawable.ic_baseline_check_24);
                    int intrinsicWidth = Objects.requireNonNull(deleteIcon).getIntrinsicWidth();
                    int intrinsicHeight = deleteIcon.getIntrinsicHeight();
                    ...
                    if (direction == DIRECTION_LEFT) {
                        ...
                        bg.setColor(getColor(R.color.colorPrimary));
                        bg.setBounds(itemView.getLeft(), itemView.getTop(), itemView.getRight(), itemView.getBottom());
                        bg.setCornerRadius(22f);
                        bg.draw(c);
...
//                        mainArticleAdapter.setOnRecyclerViewItemClickListener(this);
        ...
        educationSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (PreferenceManager.getInstance().getEducationSettings(PreferenceManager.PREF_EDUCATION_TOGGLE) != isChecked) {
    ...
    private void checkList() {
        if (totalItems >= 1) {
            emptyText.setAlpha(1f);
            emptyLottie.setAlpha(1f);

            emptyLottie.animate().alpha(0f).start();
            emptyText.animate().alpha(0f).start();
            ...
            emptyLottie.setAlpha(0f);

            emptyLottie.playAnimation();
            emptyLottie.setVisibility(View.VISIBLE);
            emptyText.setVisibility(View.VISIBLE);

            emptyLottie.animate().alpha(1f).start();
            emptyText.animate().alpha(1f).start();
        }
    }
